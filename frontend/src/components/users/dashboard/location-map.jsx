...
export function LocationMap() {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      markerRef.current = L.marker([coordinates.lat || 0, coordinates.lng || 0])
        .addTo(mapRef.current)
        .bindPopup(address || "Use the button to get your location")
        .openPopup()
    })
  ...
  // Update marker whenever coordinates or address change
  useEffect(() => {
    if (!mapRef.current || coordinates.lat == null || coordinates.lng == null) return
    ...
    import("leaflet").then((L) => {
      ...
      markerRef.current = L.marker([coordinates.lat, coordinates.lng])
        .addTo(mapRef.current)
        .bindPopup(address || "Current location")
        .openPopup()
  ...
  const fetchCurrentLocation = async () => {
    ...
    try {
      ...
      console.error(err)
      alert("Unable to fetch location or address. Check permissions.")
      dispatch(setAddress("Error fetching address"))
    } finally {
      setLoading(false)
    }
  }
          ...
          <CardContent className="space-y-4 pt-4">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="p-3 rounded-lg bg-white/10 border border-white/10">
                <p className="text-sm break-words">{address || "Use the button to fetch your location"}</p>
              </div>

              <div className="flex gap-2">
                <Badge variant="outline" className="bg-transparent border-white/20">
                  Lat: {coordinates?.lat?.toFixed(4) || "0.0000"}
                </Badge>
                <Badge variant="outline" className="bg-transparent border-white/20">
                  Lng: {coordinates?.lng?.toFixed(4) || "0.0000"}
                </Badge>
              </div>
            </div>
          </CardContent>
